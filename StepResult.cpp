...
{
Done::Done(std::unique_ptr<void, TypeErasedDeleter> return_value, std::vector<std::unique_ptr<Task>> child_tasks)
...
{}
Ready::Ready(bool high_priority)
...
Ready::Ready(std::vector<std::unique_ptr<Task>> child_tasks)
    : child_tasks(std::move(child_tasks))
{}
WaitForWaker::WaitForWaker(Waker &waker) 
    : waker(waker) 
{}
...
Wait::Wait(OnWaitFinish on_wait_finish, std::variant<WaitForWaker, WaitForChildTasks> wait_for)
    : on_wait_finish(on_wait_finish), wait_for(std::move(wait_for))
{}
Wait::Wait(OnWaitFinish on_wait_finish, std::vector<std::unique_ptr<Task>> child_tasks)
