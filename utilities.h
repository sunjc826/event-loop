...
struct TypeErasedDeleter
{
    ...
    TypeErasedDeleter(void (*deleter)(void *)) : deleter(deleter) {}

    template <typename T>
    static TypeErasedDeleter create()
    {
        return TypeErasedDeleter([](void *p)
...
template <typename T>
std::unique_ptr<void, TypeErasedDeleter>
make_type_erased(std::unique_ptr<T> ptr)
{
    std::unique_ptr<void, TypeErasedDeleter> type_erased_ptr(
        ptr.get(), TypeErasedDeleter::create<T>());
    ptr.release();
